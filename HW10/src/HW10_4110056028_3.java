public class HW10_4110056028_3 extends Buy_Phone {
    ...
    private void mergeSort(final int[][] A, final int start, final int len) {
        ...
        mergeSort(A, start + MID, len - MID);
        final int LSTART = start, LCOUNT = MID, RSTART = start + MID, RCOUNT = len - MID, LO = LSTART + LCOUNT,
                HI = RSTART + RCOUNT;
        int i = LSTART, j = RSTART, k = LSTART;
        while (i < LO || j < HI)
            TMP[k++] = A[i < LO && j < HI ? A[j][0] > A[i][0] || A[j][1] > A[i][1] ? i++ : j++
                    : i < LO ? i++ : j++];
